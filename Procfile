web: sh -c "echo 'PGHOST='$PGHOST && echo 'PGDATABASE='$PGDATABASE && python manage.py migrate && python manage.py collectstatic --noinput && gunicorn --bind 0.0.0.0:$PORT --workers 3 huds_project.wsgi:application"
